...
- (void)viewDidLoad
{
    ...
    if ([[NLUtils getAgenttypeid] isEqualToString:@"0"] || [NLUtils getAgenttypeid] == nil)
    {
        //初始化视图
        [self initView];
    }else
...
//    [self loadDataOfAgenturl];
}

-(void)loadDataOfAgenturl
{
    [LoadDataWithASI loadDataWithMsgbody:nil apiName:@"ApiAgentApply" apiNameFunc:@"insertParttimeagent" rolePath:@"//operation_response/msgbody" type:PublicCommon completionBlock:^(NSDictionary *data, NSError *error) {
        NSLog(@"mobile %@",data);
    }];
}
...
- (void)getDataOfAgentNotify:(NLProtocolResponse *)response
{
    ...
    else
    {
        ...
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"申请成功" message:[NSString stringWithFormat:@"恭喜您，您的代理商体验资格申请成功，您的代理商代号为：%@",message] delegate:self cancelButtonTitle:@"体验" otherButtonTitles:nil, nil];
        [alertView show];
    }
}
