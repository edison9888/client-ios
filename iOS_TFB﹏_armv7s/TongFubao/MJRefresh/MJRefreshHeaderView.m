...
- (void)setState:(RefreshState)state
{
	...
	switch (_state = state) {
		...
		case RefreshStateRefreshing:
        {
            ...
            [UIView animateWithDuration:0.2 animations:^{
                // 1.顶部多出65的滚动范围
                UIEdgeInsets inset = _scrollView.contentInset;inset.top = _scrollViewInitInset.top + kViewHeight;
//                inset.top = kViewHeight;
                _scrollView.contentInset = inset;
                // 2.设置滚动位置
                _scrollView.contentOffset = CGPointMake(0,- _scrollViewInitInset.top -kViewHeight);
            }];
			break;
        }
	}
