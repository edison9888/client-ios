...
@property (weak, nonatomic) IBOutlet UILabel *lablePhone; /*账户显示*/
@property (weak, nonatomic) IBOutlet UIView *viewA;
@property (weak, nonatomic) IBOutlet UIImageView *answer1View;
...
//    
//    self.Answer1.infoText.leftView = paddingView1;
    
//    self.Answer1.infoText.leftViewMode = UITextFieldViewModeAlways;
//    
//    [self.Scroller addSubview:_Answer1];
    
    /*
     UIView *infoBasicView = [UIView viewWithFrame:CGRectMake(16, 104, 288, 40)];
     infoBasicView.layer.borderColor = [UIColor grayColor].CGColor;
     infoBasicView.layer.contents = (__bridge  id) [UIImage imageNamed:@"input_fieldside.png"].CGImage;
     [self.view addSubview:infoBasicView];
     */
    
    /*输入密保问题页面改进 弹框的引用问题 以后再改*/
  
    answer1Btn=  [[TextfiledView alloc] initWithFrame:CGRectMake( 16,  94 , 288, 40 )];
    answer1Btn.infoText.placeholder  = @"回答密保问题1";
    answer1Btn.infoText.delegate     = self;
    answer1Btn.infoText.tag          = 3838;
    answer1Btn.infoText.font         = [UIFont systemFontOfSize:15.0];
    answer1Btn.layer.contents = (__bridge  id) [UIImage imageNamed:@"input_fieldside.png"].CGImage;
    ...
    if (UserBool==YES) {
        ...
        self.navigationController.topViewController.title= @"核对密保问题";
        ...
        NSString * num =[NSString stringWithFormat:@"%f",answer1Btn.frame.origin.y+25];
        [answer1Btn.layer setValue:num forKeyPath:@"frame.origin.y"];
        
        num =[NSString stringWithFormat:@"%f",_viewA.frame.origin.y+10];
        [_viewA.layer setValue:num forKeyPath:@"frame.origin.y"];
        
        num =[NSString stringWithFormat:@"%f",_Btn1.frame.origin.y+20];
        [_Btn1.layer setValue:num forKeyPath:@"frame.origin.y"];
        
        num =[NSString stringWithFormat:@"%f",_Icon1.frame.origin.y+20];
        [_Icon1.layer setValue:num forKeyPath:@"frame.origin.y"];
...
- (void)setOn:(BOOL)on animated:(BOOL)animated{
    ...
    if (_Switch.on==NO) {
        ...
        _Answer2.placeholder= @"回答密保问题2";
        _Answer2.frame= CGRectMake(16, 250, 288, 40);
        _Answer3= [[UITextField alloc]init];
        [_Answer3 setBackground:[UIImage imageNamed:@"input_fieldside.png"]];
        _Answer3.placeholder= @"回答密保问题3";
        _Answer3.frame= CGRectMake(16, 370, 288, 40);
...
-(void)showAlert{
    ...
    if (UserBool==YES) {
        ...
        self.alert = [MLTableAlert tableAlertWithTitle:@"选择密保问题" cancelButtonTitle:@"取消" numberOfRows:^NSInteger (NSInteger section)
                      {
                          if ([myArray count]== 0 ){
                              [self showErrorInfo:@"数据异常、请稍后再试" status:NLHUDState_Error];
                              return 0;
...
- (IBAction)OnclickBtn:(id)sender {
    ...
    if (answer1Btn.infoText.text.length>=1) {
        ...
        if (UserBool!=YES) {
            ...
            for (NSDictionary *answerQueDic in queArray) {
                ...
                if ([_Btn1.titleLabel.text isEqualToString:answerQueDic[@"que"]]) {
                    if ([answer1Btn.infoText.text isEqualToString:answerQueDic[@"answer"]]) {
...
- (void)SecretTextSettingURL{
    ...
    NSString *answer1= answer1Btn.infoText.text;
