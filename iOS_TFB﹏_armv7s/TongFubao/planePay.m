...
-(void)createInforForResultView:(NLTransferResultViewController*)vc
{
    ...
    switch (self.myViewYiBaoType)
    {
        case YiBaoPayType_phone:
        {
            ...
            [arr addObject:dic];
            dic = [NSDictionary dictionaryWithObjectsAndKeys:@"充值金额",@"header", [NSString stringWithFormat:@"%@元",_payRechamoneyStr],@"content", nil];
            [arr addObject:dic];
            dic = [NSDictionary dictionaryWithObjectsAndKeys:@"支付金额",@"header", [NSString stringWithFormat:@"%@元",_paymoneyStr],@"content", nil];
            [arr addObject:dic];
        }
            break;
        case YiBaoPayType_Qcoin:
        {
            ...
            dic = [NSDictionary dictionaryWithObjectsAndKeys:@"付款人",@"header", _payName.text,@"content", nil];
            [arr addObject:dic];
            dic = [NSDictionary dictionaryWithObjectsAndKeys:@"充值QQ",@"header", _cardInfo.qNumber
                   ,@"content", nil];
            [arr addObject:dic];
            dic = [NSDictionary dictionaryWithObjectsAndKeys:@"充值金额",@"header", [NSString stringWithFormat:@"%@元",_cardInfo.payMoney],@"content", nil];
            [arr addObject:dic];
            dic = [NSDictionary dictionaryWithObjectsAndKeys:@"支付金额",@"header", [NSString stringWithFormat:@"%@元",_cardInfo.payMoney],@"content", nil];
            [arr addObject:dic];
        ...
        case YiBaoPayType_ZhuanZhang:
        {
            ...
            dic = [NSDictionary dictionaryWithObjectsAndKeys:@"手续费",@"header", [NSString stringWithFormat:@"%@元",[self.myDictionary objectForKey:@"payfee"]],@"content", nil];
            [arr addObject:dic];
            
            dic = [NSDictionary dictionaryWithObjectsAndKeys:@"支付金额",@"header", [NSString stringWithFormat:@"%@元",[self.myDictionary objectForKey:@"money"]],@"content", nil];
            [arr addObject:dic];
        }
            break;
        ...
        case YiBaoPayType_CreditCardPayments:
        {
            ...
            dic = [NSDictionary dictionaryWithObjectsAndKeys:@"支付金额",@"header", [NSString stringWithFormat:@"%@元",_paymoney.text],@"content", nil];
            [arr addObject:dic];
        ...
        case YiBaoPayType_Merchantsgathering:
        {
            ...
            dic = [NSDictionary dictionaryWithObjectsAndKeys:@"付款金额",@"header", [NSString stringWithFormat:@"%@元",_payRechamoneyStr],@"content", nil];
            [arr addObject:dic];
...
-(void)ApiAuthorKuaibkcardInfoAdd
{
    ...
    NSString *bkcardisdefaultPaymentStr;
    
    if (_btnSayBankCard.selected==YES) {
         bkcardisdefaultPaymentStr = @"0";
         bkcardisdefaultStr= @"0";
    }
   
    if (_rememberBtn.selected == YES)
    {
         bkcardisdefaultPaymentStr = @"1";
         bkcardisdefaultStr= @"1";
    }
    
    NSString* name = [NLUtils getNameForRequest:Notify_ApiAuthorKuaibkcardInfoDefault];
    REGISTER_NOTIFY_OBSERVER(self, ApiAuthorKuaibkcardInfoAddNotify, name);
    [[[NLProtocolRequest alloc] initWithRegister:YES] getApiAuthorKuaibkcardInfoAdd:bankId bkcardbank:_bankList.titleLabel.text bkcardno:_paycard.text bkcardbankman:manName bkcardbankphone:payPhone bkcardyxmonth:expireMonthStr bkcardyxyear:expireYearStr bkcardcvv:cvv bkcardidcard:manCardId bkcardcardtype:@"creditcard" bkcardisdefault:bkcardisdefaultStr bkcardisdefaultPayment:bkcardisdefaultPaymentStr];/*雨*/
}
