...
static const CGFloat kHeightOfTopScrollView = 41.0f;
...
- (void)createNameButtons
{
    ...
    //顶部tabbar的总长度
    //    CGFloat topScrollViewContentWidth = 320;
    //每个tab偏移量
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        //累计每个tab文字的长度
        //        topScrollViewContentWidth += kWidthOfButtonMargin+106;
        
        
        if([_viewArray count]==2){
            //设置按钮尺寸
            [button setFrame:CGRectMake(xOffset,-64,
                                        159, kHeightOfTopScrollView)];
            //计算下一个tab的x偏移量
            xOffset += 159 + kWidthOfButtonMargin;

        }else{
            //设置按钮尺寸
            [button setFrame:CGRectMake(xOffset,-64,
                                        106, kHeightOfTopScrollView)];
            //计算下一个tab的x偏移量
            xOffset += 106 + kWidthOfButtonMargin;
        }
        
                
        [button setTag:i+100];
        if (i == 0) {
            //            _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin, 0, 106, _shadowImage.size.height);
            button.selected = YES;
        }
        ...
        [button setBackgroundImage:_shadowImage forState:UIControlStateSelected];
    ...
    //设置顶部滚动视图的背景图
    _topScrollView.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"top_background.png"]];
