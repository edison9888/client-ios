...
@implementation WaterEleBillSure
...
-(void)UIInit
{
    ...
    UILabel *companyNameText = [[UILabel alloc]initWithFrame:CGRectMake(15, 46+IOS7HEIGHT, 100, 15)];
    companyNameText.backgroundColor = [UIColor clearColor];
    companyNameText.textColor = [UIColor grayColor];
    companyNameText.text = @"缴费单位";
    companyNameText.font = [UIFont systemFontOfSize:14];
    [self.view addSubview:companyNameText];
    
    UILabel *companyName = [[UILabel alloc]initWithFrame:CGRectMake(115, 46+IOS7HEIGHT, 190, 15)];
    companyName.backgroundColor = [UIColor clearColor];
    companyName.textColor = [UIColor grayColor];
    companyName.textAlignment = UITextAlignmentRight;
    companyName.text = _information[@"companyName"];
    ...
    customerNum.text = _information[@"username"];
    ...
    payMoney.text = [NSString stringWithFormat:@"￥%@",_information[@"totalBill"]];
...
-(void)payBill
{
    ...
    NSString* name = [NLUtils getNameForRequest:Notify_waterEle_submitOrder];
    
    REGISTER_NOTIFY_OBSERVER(self, payMoneyRqNotify, name);
    ...
    [[[NLProtocolRequest alloc] initWithRegister:YES]submitWaterEleOrder:_information[@"orderid"] paycardid:_paycardid rechabkcardno:_cardno merReserved:merReserved];
}

-(void)payMoneyRqNotify:(NSNotification*)notify
{
    NLProtocolResponse* response = (NLProtocolResponse*)notify.object;
    int error = response.errcode;
...
-(void)completeOrder
{
    NSString* name = [NLUtils getNameForRequest:Notify_waterEle_completeOrder];
    
    REGISTER_NOTIFY_OBSERVER(self, completeOrderNotify, name);
    
    [[[NLProtocolRequest alloc] initWithRegister:YES] completeWaterEleOrder:_information[@"orderid"] bkntno:_bkntno];
...
-(void)doCompleteOrderNotifyNotify:(NLProtocolResponse*)response
{
    ...
    else
    {
        ...
        payOk.title = @"缴费成功";
        payOk.dict = @{BUY_SUC_TITLT: @"缴费成功",BUY_SUC_CONTENT: @"恭喜您通过通付宝缴费成功"};
