...
- (void)initTableViewHead
{
    ...
    for(int i = 0; i < 10; i++)
    {
		RMHeard *headView = [[RMHeard alloc] init];
...
-(NSMutableArray*)messagesArray {
    if (!_messagesArray) {
        ...
        [_messagesArray addObject:@{ @"sender" : @"Jonathan Ive", @"message" : @"sI can't believe what people are saying about those iOS 7 icons! Are they missing aluminium or something?" }];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    RMHeard *headView = headViews[section];
    
    return headView.backBtn.selected? [[self.messagesArray valueForKey:@"sender"] count] : 0;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.text = [self.messagesArray objectAtIndex:indexPath.section][@"sender"];
