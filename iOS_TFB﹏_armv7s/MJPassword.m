...
@interface MJPassword()

@property (nonatomic,retain) MJPath* pathLayer;

- (void) setLayerFrames;

@end

@implementation MJPassword

- (id)initWithFrame:(CGRect)frame
{
    int IOS7IPHONE4=IOS7_OR_SCREEN==YES?0.8:1;
    
    self = [super initWithFrame:CGRectMake(0, 40*IOS7IPHONE4, 280, 440)];
    if (self)
    {
        //normal
        ...
        self.pathLayer = [MJPath layer];
        self.pathLayer.password = self;/*画线的*/
...
- (void) setLayerFrames
{
    ...
    for (int i = 0; i < 3; i++)
    {
        for (int j = 0; j < 3; j++)
        {
            CGFloat x = kCircleLeftTopMargin+kCircleRadius+j*(kCircleRadius*2+kCircleBetweenMarginMore*2);
            CGFloat y = ToTopMargin+kCircleRadius+i*(kCircleRadius*2+kCircleBetweenMarginMore*2);
