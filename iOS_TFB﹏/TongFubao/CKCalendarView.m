...
- (void)_init:(CKCalendarStartDay)firstDay {
    ...
    [btnyes addTarget:self action:@selector(btn:) forControlEvents:UIControlEventTouchUpInside];
    [btnyes setBackgroundColor:[UIColor lightGrayColor]];
...
- (void)layoutSubviews {
    ...
    newFrame.size.height = containerHeight + CALENDAR_MARGIN + TOP_HEIGHT+70;
    self.frame = newFrame;

    if (numberOfWeeksToShow==6) {
        [btnyes setFrame:CGRectMake(60, 320 , 170, 44)];
    }else{
        [btnyes setFrame:CGRectMake(60, 280, 170,44)];
    }
...
- (void)_setDefaultStyle {
    self.backgroundColor = RGBACOLOR(159, 192, 215, 1);
