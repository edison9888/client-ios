...
/***
 读取银行列表(New分页显示)
 */
-(void)readBankListByPaging:(NSString*)activemobilesms msgstart :(NSString*)msgstart msgdisplay:(NSString*)msgdisplay querywhere:(NSString*)querywhere
{
    BOOL send = [self preRequest:NLProtocolRequest_readBankList name:Notify_readBankList];
    if (!send)
    {
        NLProtocolData* pd = [[NLProtocolXML shareProtocolXML] readBankListByPagingXML:activemobilesms msgstart:msgstart msgdisplay:msgdisplay querywhere:querywhere];
        [_request/*[[ProtocolRequest alloc] init]*/ startRequest:pd action:Notify_readBankList];
    }
}
