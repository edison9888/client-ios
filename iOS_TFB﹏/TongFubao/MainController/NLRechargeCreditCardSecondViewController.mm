...
@interface NLRechargeCreditCardSecondViewController ()
{
    NSString* _bankname;
    NSString* _bankID;
    NSString* _cardno;
...
@implementation NLRechargeCreditCardSecondViewController
...
//    self.navigationController.topViewController.title = @"信用卡充值(2)";
    [self initValue];
    [self initVisaReader];
...
-(void)createInforForResultView:(NLTransferResultViewController*)vc
{
    ...
    [arr addObject:dic];
    dic = [NSDictionary dictionaryWithObjectsAndKeys:@"付款金额",@"header", _paymoney,@"content", nil];
    [arr addObject:dic];
