...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //if (self.bankIndex != indexPath.row)
    {
        ...
        if ([_delegate respondsToSelector:@selector(dataSearch:didSelectWithObject:withState:)])
        {
            ...
            int  bankID = [[[self.myArray objectAtIndex:indexPath.row] objectForKey:@"bankno"] intValue];
            //NSString* name = node.myName;
            //int bankID = [node.myID intValue];
            [_delegate dataSearch:self didSelectWithObject:name withState:bankID];
        }
    }
...
-(void)hidMyHUD:(BOOL)success
{
    ...
    else
    {
        self.myHUD.customView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"unCheckmark.png"]] ;
        self.myHUD.mode = MBProgressHUDModeCustomView;
        self.myHUD.labelText = @"网络异常，请检查网络";
    }
    //sleep(2);
    [self.myHUD hide:YES afterDelay:2];
}
...
-(void)readOrderUrl{
    ...
    [[[NLProtocolRequest alloc] initWithRegister:YES]readBankListByPaging:self.myActivemobilesms msgstart:_msgstart msgdisplay:_msgdisplay querywhere:_querywhere];
