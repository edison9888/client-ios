...
- (void)viewDidLoad
{
    ...
    self.navigationController.topViewController.title = @"信用卡还款";
    [self initValue];
    [self initVisaReader];
...
- (IBAction)onNextBtnClicked:(id)sender
{
    ...
    NLCreditCardSecondViewController* vc = [[NLCreditCardSecondViewController alloc] initWithNibName:@"NLCreditCardSecondViewController"
                                                                                              bundle:nil];
    vc.myDictionary = [NSDictionary dictionaryWithObjectsAndKeys:_paytype,@"paytype",_paymoney,@"paymoney",_shoucardno,@"shoucardno",_shoucardmobile,@"shoucardmobile",_shoucardman,@"shoucardman",_shoucardbank,@"shoucardbank", nil];
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.row)
    {
        ...
        case TableViewCardNunberRowType:
        {
            ...
            {
                cell.myTextField.placeholder = @"刷卡输入卡号";
            }
            else
            {
