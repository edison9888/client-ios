...
@synthesize TicketBillsArray,allPriceBillsArray,textString,TicketBillFlightInformation,sureInfoArray,TicketBillId,TicketContactArray,perSonIdArray,ContactIdArray,OrderId,verify,styGoBack;
...
-(void)ApipayCardCheck
{
    ...
    [[[NLProtocolRequest alloc] initWithRegister:YES]getApipayCardCheck:paycardkey_check bkcardno:bkcardno_check paytype:paytype_check];
}
...
-(void)suerPayMoney
{
    ...
    if (BUTTONSTATE == YES && ![self.textString isEqualToString:_carText.text] && ([_carText.text length] == 19 || [_carText.text length] == 16))
    {
        _scrollView.frame = CGRectMake(0, -64, self.view.frame.size.width, self.view.frame.size.height+64);
...
//        planePayView.TICKETSTY = YES;
    ...
    else if (BUTTONSTATE == YES && PushState == YES && [self.textString isEqualToString:_carText.text]  && ([_carText.text length] == 19 || [_carText.text length] == 16))
    {
        [self orderNumberVerificationCode];
    }
}
...
//    self.TicketBillId
    ...
    // 票,乘机人,联系人,卡
    [[[NLProtocolRequest alloc]initWithRegister:YES] TicketBillId:self.TicketBillId  backTicketId:self.backTicketId  styGoBack:self.styGoBack perSonIdArray:self.perSonIdArray ContactIdArray:self.ContactIdArray   payinfoCardInfoArray:self.sureInfoArray];
