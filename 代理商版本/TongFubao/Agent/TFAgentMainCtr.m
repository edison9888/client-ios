...
-(void)dataInit
{
    _todayEarnStr = [NSString stringWithFormat:@"%.2lf",(arc4random()%92) + 32.23 ]; //本日收益
    _totalEarnStr = [NSString stringWithFormat:@"%.2lf",(arc4random()%192) + 232.23 ]; //历史总收益
    _monthSellNumStr = [NSString stringWithFormat:@"%d",(arc4random()%68) + 32 ];  //本月出售设备
    _totalDeviceNumStr = [NSString stringWithFormat:@"%d",(arc4random()%172) + 53 ]; //总设备
    _totalUserNumStr= [NSString stringWithFormat:@"%d",(arc4random()%142) + 26 ];
}

/*随机添加*/
-(void)bgTappedAction:(UITapGestureRecognizer *)tap
{
    _todayEarn.text= [self getRandomNumber:99 Number:79 strint:_todayEarn.text tag:0.17];
    _totalEarn.text= [NSString stringWithFormat:@"本区历史总收益%@",[self getRandomNumber:3999 Number:129 strint:_todayEarn.text tag:0.12]];
    
    _monthSellNum.text= [self getRandomNumber:20 Number:45 strint:_monthSellNum.text tag:1];
    _totalDeviceNum.text= [self getRandomNumber:150 Number:21 strint:_totalDeviceNum.text tag:1];
    _totalUserNum.text= [self getRandomNumber:150 Number:29 strint:_totalDeviceNum.text tag:1];
}

/*添加上次*/
-(NSString*)getRandomNumber:(int)num Number:(int)andnum strint:(NSString*)strint tag:(float)tag
{
    if (tag!=1) {
       return [NSString stringWithFormat:@"%.2lf",(arc4random()%num) + num + [strint intValue]*tag+[strint intValue]];
    }else
       return [NSString stringWithFormat:@"%d",(arc4random()%num) + num + [strint intValue]];
}
...
//    [titleView addSubview:titleBtn];
    ...
    self.navigationController.view.backgroundColor = RGBACOLOR(0, 102, 156, 1.0);
    ...
    [yellowView addGestureRecognizer:bgTap];
    ...
    bgTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(bgTappedAction:)];
    [greenView addGestureRecognizer:bgTap];
    ...
    [pinkView addTarget:self action:@selector(bgTappedAction:) forControlEvents:UIControlEventTouchUpInside];
    bgTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(bgTappedAction:)];
    [pinkView addGestureRecognizer:bgTap];
    ...
    [addGoodsBt addTarget:self action:@selector(bgTappedAction:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [searchBt addTarget:self action:@selector(bgTappedAction:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)doReadagentinfoNotify:(NLProtocolResponse *)response
{
    ...
    else
    {
        ...
        //本区用户数/销售数量 ==1
        _monthSellNum.text = agentTypeId != 1? _monthSellNumStr : _totalUserNumStr;
