...
- (void)initData
{
    ...
    [LoadDataWithASI loadDataWithMsgbody:dic apiName:@"ApiAuthorInfo" apiNameFunc:@"GetpaycardList" rolePath:@"//operation_response/msgbody/msgchild" type:PublicCommon completionBlock:^(id data, NSError *error) {
        ...
        if (data)
        {
            ...
            //当前是否有授权码
            if ([userInfoDic objectForKey:@"paycardid"])
            {
                //我的授权码
...
- (IBAction)bindingAction:(id)sender
{
    ...
    numberView.impowerNumber = [userInfoDic objectForKey:@"paycardid"]? [userInfoDic objectForKey:@"paycardid"] : @"";
    [self.navigationController pushViewController:numberView animated:YES];
    
//    MyImpowerController *impowerView = [[MyImpowerController alloc] initWithType:([sender isMemberOfClass:[UIButton class]]? ImpowerType : ImpowerInfoType)];
//    [self.navigationController pushViewController:impowerView animated:YES];
}
