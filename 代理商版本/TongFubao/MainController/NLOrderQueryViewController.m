...
-(NLButtonViewController*)getButton:(NSString*)title tag:(int)tag
{
    ...
    NLButtonViewController* vc = [[NLButtonViewController alloc] initWithTitle:title image:[UIImage imageNamed:[[self.myArray objectAtIndex:tagNum] objectForKey:@"com"]] tag:tag];
    vc.myDelegate = self;
    return vc;
}
...
-(void)doReadMenuModuleNotify:(NLProtocolResponse*)response
{
    ...
    for (int i=0; i<[comname count]; i++)
    {
        ...
        dic = [NSDictionary dictionaryWithObjectsAndKeys:comidStr,@"comid",comStr,@"com",comnameStr,@"comname",apitypeStr,@"apitype",comlogoStr,@"comlogo", nil];
