...
//                    _DefualCared.selected= NO;
                    ...
                    [_OnBtnClick.layer setValue:@10 forKeyPath:@"frame.origin.y"];
                    
                    /*是否有默认卡号*/
                    if (flagnot==YES)
                    {
                        [_TextFiledCared.layer setValue:@414 forKeyPath:@"frame.origin.y"];
                         [_viewBtn.layer setValue:@475 forKeyPath:@"frame.origin.y"];
                    }
                    else
                    {
...
-(void)YIBAOzhuanzhangTophone
{
    ...
    */
    ...
    [[[NLProtocolRequest alloc] initWithRegister:YES]getApiYiBaoPhonePay:strRechamoney payMoney:PhoneGiveStr2  rechargePhone:PhoneNum bankCardId:bkcardnoCheckStr bankId:bkcardbankidsStr manCardId:bkcardidcardStr payPhone:bkcardbankphonesStr manName:bkcardbankmansStr expireYear:bkcardyxyearsStr expireMonth:bkcardyxmonthsStr cvv:bkcardcvvsStr mobileProvince:mobileProvince paycardid:_paycardid];
    [self showErrorInfo:@"请稍候" status:NLHUDState_None];
}
